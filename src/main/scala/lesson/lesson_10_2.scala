...
class ODEFunc[ParamType <: FloatNN: Default](hidden_dim: Int)
...
// 定义网络结构
class SimpleGNNLayer[ParamType <: FloatNN: Default](in_features: Int, out_features: Int)
    extends TensorModule[ParamType]
    with HasParams[ParamType] {
  ...
  def forward(input: Tensor[ParamType], edge_index: Tensor[Int64]): Tensor[ParamType] = {
    ...
    // 将原始边与自环拼接
    val edge_index_with_self_loops =
      torch.cat(Seq(edge_index, self_loopss.to(torch.int64)), dim = 1)
