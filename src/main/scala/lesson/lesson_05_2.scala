...
// 为了演示，我们创建一个简单的虚拟数据集：
class DummyDataset[Input <: BFloat16 | FloatNN: Default, Target <: BFloat16 | FloatNN: Default](
    num_samples: Int = 100
  ...
  val labels = torch
    .randint(0, 2, Seq(num_samples))
    .to(dtype = implicitly[Default[Target]].dtype) // 示例：100 个二元标签
  ...
  override def iterator: Iterator[(Tensor[Input], Tensor[Target])] =
    ??? // (0 until this.length).iterator.map(getItem)
}
...
object lesson_05_2 {
  ...
  // 自定义collate函数
  def pad_collate(
      batch: Seq[(Tensor[Float32], Tensor[Int64])]
...
//      }
    ...
    val train_loader_drop_last =
      DataLoader(dataset = dataset, batch_size = 32, shuffle = true, drop_last = true)
...
//        val batch = train_loader_drop_last(i)
    ...
    val gpu_optimized_loader = DataLoader(
      dataset = dataset,
      batch_size = 32,
      shuffle = true,
      num_workers = 4,
...
//        var (features, labels) = batch
    ...
    // 为每个样本计算权重
    val targets =
      Seq(900, 100) // torch.randint(0, 2, Seq(900,100)).to(torch.int64) // 示例：1000 个二元标签
    ...
    // 创建采样器
    val sampler =
      WeightedRandomSampler(weights = sample_weights, num_samples = num_samples, replacement = true)
