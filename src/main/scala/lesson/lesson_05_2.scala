...
//  val labels = torch.randint(0, 2, Seq(num_samples)).to(Target) // 示例：100 个二元标签
//
...
//
//// 返回可变长度张量的示例Dataset
//class VariableSequenceDataset[Input <: BFloat16 | FloatNN: Default, Target <: BFloat16 | FloatNN: Default](data: Tensor[Input]*)
...
//
//    // 堆叠标签（假设它们是简单的标量）
//    val labels_tensor = torch.tensor(labels)
...
//      for( i <- (0 to train_loader_drop_last.size)){
...
//
//      // 在训练循环内部（假设你有 GPU）
//      for (i <- (0 to gpu_optimized_loader.size)) {
...
//      val sample_weight = targets.map(t =>  {
//        val t1 =1.0  / class_counts(t)
//        t1
//      }) //.toArray
//      val sample_weights = torch.tensor(sample_weight )
//
//      // 创建采样器
