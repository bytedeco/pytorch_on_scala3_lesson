...
object SimpleGPU {
  ...
  def FAIL_IF_ERR(err: TRITONSERVER_Error , MSG: String): Unit = {
    if (err != null) {
      System.err.println("error: " + MSG + ":" + TRITONSERVER_ErrorCodeString(err) + " - " + TRITONSERVER_ErrorMessage(err))
    ...
    protected class FreeDeallocator(p: Pointer) extends Pointer(p) with Pointer.Deallocator {
      override def deallocate(): Unit = {
        if (!isNull) {
          ...
          if (cuerr != cudaSuccess) System.err.println("error: failed to get CUDA pointer attribute of " + this + ": " + cudaGetErrorString(cuerr).getString)
          if (attr.`type` == cudaMemoryTypeDevice) cuerr = cudaFree(this)
          else if (attr.`type` == cudaMemoryTypeHost) cuerr = cudaFreeHost(this)
          if (cuerr != cudaSuccess) System.err.println("error: failed to release CUDA pointer " + this + ": " + cudaGetErrorString(cuerr).getString)
        }
      }
    }
  }
  ...
  object TRITONSERVER_ServerDeleter {
    protected class DeleteDeallocator(p: Pointer) extends TRITONSERVER_Server(p) with Pointer.Deallocator {
      override def deallocate(): Unit = {
  ...
  def Usage(msg: String): Unit = {
    ...
    System.err.println("\t-m <\"system\"|\"pinned\"|gpu>" + " Enforce the memory type for input and output tensors." + " If not specified, inputs will be in system memory and outputs" + " will be based on the model's preferred type.")
  ...
  class ResponseAlloc extends TRITONSERVER_ResponseAllocatorAllocFn_t {
    override def call(allocator: TRITONSERVER_ResponseAllocator, tensor_name: String, byte_size: Long, preferred_memory_type: Int, preferred_memory_type_id: Long, userp: Pointer, buffer: PointerPointer[? <: Pointer], buffer_userp: PointerPointer[? <: Pointer], actual_memory_type: IntPointer, actual_memory_type_id: LongPointer): TRITONSERVER_Error = {
      ...
      if (byte_size == 0) {
        ...
        actual_memory_type.get match {
          case TRITONSERVER_MEMORY_CPU_PINNED =>
            ...
            if ((err != cudaSuccess) && (err != cudaErrorNoDevice) && (err != cudaErrorInsufficientDriver)) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INTERNAL, "unable to recover current CUDA device: " + cudaGetErrorString(err).getString)
            err = cudaHostAlloc(allocated_ptr, byte_size, cudaHostAllocPortable)
            if (err != cudaSuccess) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INTERNAL, "cudaHostAlloc failed: " + cudaGetErrorString(err).getString)
          case TRITONSERVER_MEMORY_GPU =>
            var err = cudaSetDevice(actual_memory_type_id.get.toInt)
            if ((err != cudaSuccess) && (err != cudaErrorNoDevice) && (err != cudaErrorInsufficientDriver)) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INTERNAL, "unable to recover current CUDA device: " + cudaGetErrorString(err).getString)
            err = cudaMalloc(allocated_ptr, byte_size)
            if (err != cudaSuccess) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INTERNAL, "cudaMalloc failed: " + cudaGetErrorString(err).getString)
        ...
        if (!allocated_ptr.isNull) {
          ...
          System.out.println("allocated " + byte_size + " bytes in " + TRITONSERVER_MemoryTypeString(actual_memory_type.get) + " for result tensor " + tensor_name)
        }
      }
      null // Success
    }
  ...
  class ResponseRelease extends TRITONSERVER_ResponseAllocatorReleaseFn_t {
    override def call(allocator: TRITONSERVER_ResponseAllocator, buffer: Pointer, buffer_userp: Pointer, byte_size: Long, memory_type: Int, memory_type_id: Long): TRITONSERVER_Error = {
      var name: String = null
      if (buffer_userp != null) name = Loader.accessGlobalRef(buffer_userp).asInstanceOf[String]
      else name = "<unknown>"
      System.out.println("Releasing buffer " + buffer + " of size " + byte_size + " in " + TRITONSERVER_MemoryTypeString(memory_type) + " for result '" + name + "'")
      memory_type match {
        case TRITONSERVER_MEMORY_CPU =>
          Pointer.free(buffer)
        case TRITONSERVER_MEMORY_CPU_PINNED =>
          ...
          if (err != cudaSuccess) System.err.println("error: failed to cudaFree " + buffer + ": " + cudaGetErrorString(err))
        case TRITONSERVER_MEMORY_GPU =>
          var err = cudaSetDevice(memory_type_id.toInt)
  ...
  def ParseModelMetadata(model_metadata: JsonObject, is_int: Array[Boolean], is_torch_model: Array[Boolean]): TRITONSERVER_Error = {
...
//    import scala.collection.JavaConversions._
    ...
    for (input_element <- model_metadata.get("inputs").getAsJsonArray.asScala) {
      ...
      if (!(input.get("datatype").getAsString == "INT32") && !(input.get("datatype").getAsString == "FP32")) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_UNSUPPORTED, "simple lib example only supports model with data type INT32 or " + "FP32")
      if (seen_data_type == null) seen_data_type = input.get("datatype").getAsString
      else if (!(seen_data_type == input.get("datatype").getAsString)) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INVALID_ARG, "the inputs and outputs of 'simple' model must have the data type")
    }
//    import scala.collection.JavaConversions._
    for (output_element <- model_metadata.get("outputs").getAsJsonArray.asScala) {
      val output = output_element.getAsJsonObject
      if (!(output.get("datatype").getAsString == "INT32") && !(output.get("datatype").getAsString == "FP32")) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_UNSUPPORTED, "simple lib example only supports model with data type INT32 or " + "FP32")
      else if (!(seen_data_type == output.get("datatype").getAsString)) return TRITONSERVER_ErrorNew(TRITONSERVER_ERROR_INVALID_ARG, "the inputs and outputs of 'simple' model must have the data type")
    }
  ...
  def GenerateInputData(input0_data: Array[FloatPointer], input1_data: Array[FloatPointer]): Unit = {
  ...
  def CompareResult(output0_name: String, output1_name: String, input0: FloatPointer, input1: FloatPointer, output0: FloatPointer, output1: FloatPointer): Unit = {
    for (i <- 0 until 16) {
      System.out.println(input0.get(i) + " + " + input1.get(i) + " = " + output0.get(i))
      System.out.println(input0.get(i) + " - " + input1.get(i) + " = " + output1.get(i))
      if ((input0.get(i) + input1.get(i)) != output0.get(i)) FAIL("incorrect sum in " + output0_name)
  ...
  def Check(response: TRITONSERVER_InferenceResponse, input0_data: Pointer, input1_data: Pointer, output0: String, output1: String, expected_byte_size: Long, expected_datatype: Int, is_int: Boolean): Unit = {
    val output_data = new mutable.HashMap[String, Pointer]
    val output_count = Array(0)
    FAIL_IF_ERR(TRITONSERVER_InferenceResponseOutputCount(response, output_count), "getting number of response outputs")
    ...
    for (idx <- 0 until output_count(0)) {
      ...
      FAIL_IF_ERR(TRITONSERVER_InferenceResponseOutput(response, idx, cname, datatype, shape, dim_count, base, byte_size, memory_type, memory_type_id, userp), "getting output info")
      ...
      if ((dim_count.get != 2) || (shape.get(0) != 1) || (shape.get(1) != 16)) FAIL("unexpected shape for '" + name + "'")
      if (datatype.get != expected_datatype) FAIL("unexpected datatype '" + TRITONSERVER_DataTypeString(datatype.get) + "' for '" + name + "'")
      if (byte_size.get != expected_byte_size) FAIL("unexpected byte-size, expected " + expected_byte_size + ", got " + byte_size.get + " for " + name)
      if (enforce_memory_type && (memory_type.get != requested_memory_type)) FAIL("unexpected memory type, expected to be allocated in " + TRITONSERVER_MemoryTypeString(requested_memory_type) + ", got " + TRITONSERVER_MemoryTypeString(memory_type.get) + ", id " + memory_type_id.get + " for " + name)
      ...
      memory_type.get match {
        ...
        case TRITONSERVER_MEMORY_GPU =>
          ...
          FAIL_IF_CUDA_ERR(cudaMemcpy(odata, base, byte_size.get, cudaMemcpyDeviceToHost), "getting " + name + " data from GPU memory")
        case _ =>
          FAIL("unexpected memory type")
      }
    }
    if (is_int) CompareResult(output0, output1, new IntPointer(input0_data), new IntPointer(input1_data), new IntPointer(output_data.get(output0).get), new IntPointer(output_data.get(output1).get))
    else CompareResult(output0, output1, new FloatPointer(input0_data), new FloatPointer(input1_data), new FloatPointer(output_data.get(output0).get), new FloatPointer(output_data.get(output1).get))
  }
  ...
  def RunInference(model_repository_path: String, verbose_level: Int): Unit = {
    ...
    FAIL_IF_ERR(TRITONSERVER_ApiVersion(api_version_major, api_version_minor), "getting Triton API version")
    if ((TRITONSERVER_API_VERSION_MAJOR != api_version_major(0)) || (TRITONSERVER_API_VERSION_MINOR > api_version_minor(0))) FAIL("triton server API version mismatch")
    // Create the server...
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetModelRepositoryPath(server_options, model_repository_path), "setting model repository path")
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetLogVerbose(server_options, verbose_level), "setting verbose logging level")
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetBackendDirectory(server_options, "/opt/tritonserver/backends"), "setting backend directory")
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetRepoAgentDirectory(server_options, "/opt/tritonserver/repoagents"), "setting repository agent directory")
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetStrictModelConfig(server_options, true), "setting strict model configuration")
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerMetadata(server, server_metadata_message), "unable to get server metadata message")
    val buffer = new BytePointer(null.asInstanceOf[Pointer])
    val byte_size = new SizeTPointer(1)
    FAIL_IF_ERR(TRITONSERVER_MessageSerializeToJson(server_metadata_message, buffer, byte_size), "unable to serialize server metadata message")
    ...
    health_iters = 0
    while (!is_ready(0)) {
      FAIL_IF_ERR(TRITONSERVER_ServerModelIsReady(server, model_name, 1, is_ready), "unable to get model readiness")
      breakable {
        if (!is_ready(0)) {
          if ( {
            health_iters += 1; health_iters
      ...
      FAIL_IF_ERR(TRITONSERVER_ServerModelMetadata(server, model_name, 1, model_metadata_message), "unable to get model metadata message")
      val buffer = new BytePointer(null.asInstanceOf[Pointer])
      val byte_size = new SizeTPointer(1)
      FAIL_IF_ERR(TRITONSERVER_MessageSerializeToJson(model_metadata_message, buffer, byte_size), "unable to serialize model status protobuf")
      ...
      if (!(model_metadata.get("name").getAsString == model_name)) FAIL("unable to find metadata for model")
...
//        }
      ...
      FAIL_IF_ERR(ParseModelMetadata(model_metadata, is_int, is_torch_model), "parsing model metadata")
    }
    // Create the allocator that will be used to allocate buffers for
    // the result tensors.
    val allocator = new TRITONSERVER_ResponseAllocator(null)
    FAIL_IF_ERR(TRITONSERVER_ResponseAllocatorNew(allocator, responseAlloc, responseRelease, null /* start_fn */), "creating response allocator")
    // Inference
    val irequest = new TRITONSERVER_InferenceRequest(null)
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestNew(irequest, server, model_name, -1 /* model_version */), "creating inference request")
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetId(irequest, "my_request_id"), "setting ID for the request")
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetReleaseCallback(irequest, inferRequestComplete, null /* request_release_userp */), "setting request release callback")
    // Inputs
    val input0 = if (is_torch_model(0)) "INPUT__0"
    else "INPUT0"
    val input1 = if (is_torch_model(0)) "INPUT__1"
    else "INPUT1"
    val input0_shape = Array(1, 16)
    val input1_shape = Array(1, 16)
    val datatype = if (is_int(0)) TRITONSERVER_TYPE_INT32
    else TRITONSERVER_TYPE_FP32
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAddInput(irequest, input0, datatype, input0_shape.map(_.toLong), input0_shape.length), "setting input 0 meta-data for the request")
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAddInput(irequest, input1, datatype, input1_shape.map(_.toLong), input1_shape.length), "setting input 1 meta-data for the request")
    val output0 = if (is_torch_model(0)) "OUTPUT__0"
    else "OUTPUT0"
    val output1 = if (is_torch_model(0)) "OUTPUT__1"
    else "OUTPUT1"
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAddRequestedOutput(irequest, output0), "requesting output 0 for the request")
    ...
    if (use_cuda_memory) {
      ...
      if (requested_memory_type != TRITONSERVER_MEMORY_CPU_PINNED) {
        ...
        FAIL_IF_CUDA_ERR(cudaMemcpy(dst, input0_data, input0_size, cudaMemcpyHostToDevice), "setting INPUT0 data in GPU memory")
        FAIL_IF_CUDA_ERR(cudaMalloc(dst, input1_size), "allocating GPU memory for INPUT1 data")
        input1_gpu.reset(dst)
        FAIL_IF_CUDA_ERR(cudaMemcpy(dst, input1_data, input1_size, cudaMemcpyHostToDevice), "setting INPUT1 data in GPU memory")
      }
      else {
        val dst = new Pointer
        FAIL_IF_CUDA_ERR(cudaHostAlloc(dst, input0_size, cudaHostAllocPortable), "allocating pinned memory for INPUT0 data")
        input0_gpu.reset(dst)
        FAIL_IF_CUDA_ERR(cudaMemcpy(dst, input0_data, input0_size, cudaMemcpyHostToHost), "setting INPUT0 data in pinned memory")
        FAIL_IF_CUDA_ERR(cudaHostAlloc(dst, input1_size, cudaHostAllocPortable), "allocating pinned memory for INPUT1 data")
        input1_gpu.reset(dst)
        FAIL_IF_CUDA_ERR(cudaMemcpy(dst, input1_data, input1_size, cudaMemcpyHostToHost), "setting INPUT1 data in pinned memory")
      }
      input0_base = input0_gpu
      input1_base = input1_gpu
    }
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(irequest, input0, input0_base, input0_size, requested_memory_type, 0 /* memory_type_id */), "assigning INPUT0 data")
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(irequest, input1, input1_base, input1_size, requested_memory_type, 0 /* memory_type_id */), "assigning INPUT1 data")
    // Perform inference...
    val completed = new CompletableFuture[TRITONSERVER_InferenceResponse]
    futures.put(irequest, completed)
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetResponseCallback(irequest, allocator, null /* response_allocator_userp */ , inferResponseComplete, irequest), "setting response callback")
    ...
    Check(completed_response, input0_data, input1_data, output0, output1, input0_size, datatype, is_int(0))
    FAIL_IF_ERR(TRITONSERVER_InferenceResponseDelete(completed_response), "deleting inference response")
...
//    val completed = new CompletableFuture[TRITONSERVER_InferenceResponse]
    ...
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestRemoveAllInputData(irequest, input0), "removing INPUT0 data")
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(irequest, input0, input1_base, input1_size, requested_memory_type, 0 /* memory_type_id */), "assigning INPUT1 data to INPUT0")
    ...
    Check(completed_response, input1_data, input1_data, output0, output1, input0_size, datatype, is_int(0))
    FAIL_IF_ERR(TRITONSERVER_InferenceResponseDelete(completed_response), "deleting inference response")
    ...
    while (i < args.length) {
      args(i) match {
        case "-m" =>
          ...
          else Usage("-m must be used to specify one of the following types:" + " <\"system\"|\"pinned\"|gpu>")
        case "-r" =>
