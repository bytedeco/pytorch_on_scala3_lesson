...
class LargeTransformerBlock[ParamType <: FloatNN: Default](dim: Int, ff_dim: Int) extends nn.Module {
  // 子模块定义示例
...
class BigModel[ParamType <: FloatNN: Default](num_layers: Int, dim: Int, ff_dim: Int, vocab_size: Int) extends nn.Module {
  // 子模块定义示例
...
// --- 模拟模型和数据集 ---
class ToyModel[ParamType <: FloatNN: Default] extends nn.Module {
...
class ToyDataset[Input <: BFloat16 | FloatNN: Default, Target <: BFloat16 | FloatNN: Default](num_samples: Int = 100) extends Dataset[Input, Target] {
  ...
  val labels = torch.randn(num_samples, 1)
