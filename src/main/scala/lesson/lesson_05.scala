...
//}
//class SimpleCustomDataset[Input <: BFloat16 | FloatNN: Default, Target <: BFloat16 | FloatNN: Default](feature: NDArray[Float], label: NDArray[Int])
//  extends Dataset[Input, Target] {
//  require(feature.getSize == label.getSize, "特征和标签的长度必须相同。")
//
//
//  override def features: Tensor[Input] = torch.tensors(feature, dtype=torch.float32)
//
//  override def targets: Tensor[Target] = torch.tensors(label, dtype=torch.int64)
//
//  //  """返回样本总数。"""
...
//  //  //获取给定索引的特征和标签
//  override def getItem(idx:  Int): (Tensor[Input], Tensor[Target]) = {
//    (torch.tensors(feature(idx), dtype=torch.float32), torch.tensors(label(idx), dtype=torch.int64))
//  }
//
