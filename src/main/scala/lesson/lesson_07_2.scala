 ...
 */
class SimpleRNNModel[ParamType <: FloatNN: Default](
    input_dim: Int,
    hidden_dim: Int,
    output_dim: Int,
...
class SimpleCNNz[ParamType <: FloatNN: Default](num_classes: Int = 10)
    extends TensorModule[ParamType]
    with HasParams[ParamType] {
...
class SimpleRNN[ParamType <: FloatNN: Default](
    input_size: Int,
    hidden_size: Int,
    output_size: Int,
    num_layers: Int = 1
  ...
  ): (Tensor[ParamType], Tensor[ParamType]) = forward(x, h0)
  ...
  def forward(
    ...
    // 形状: (层数 * 方向数, 批次, 隐藏尺寸)
    val hidden =
      if !h0.isDefined then torch.zeros(num_layers, x.size(1), hidden_size).to(x.device) else h0.get
    // 数据通过RNN层
...
//  @main
  def main(): Unit = {
    ...
    val dummy_input_rnn = torch
      .randn(sequence_length, batch_size, input_features, requires_grad = false)
      .to(torch.float32)
...
//  @main
  def mains(): Unit = {
    ...
    val rnn_layer =
      nn.RNN(input_size = input_features, hidden_size = hidden_size, batch_first = true)

    // 将输入通过层（输出形状也将是批次维度在前）
    val (output, hidden_state) =
      rnn_layer(rnn_input_batch_first, None) // todo rnn remove hidden state None
    ...
    val SEQ_LEN = 15
    val dummy_input =
      torch.randn(BATCH_SIZE, SEQ_LEN, INPUT_DIM).to(torch.float32) // 形状: (batch, seq, feature)
