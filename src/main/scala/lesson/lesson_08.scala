...
class SimpleNet2[ParamType <: FloatNN: Default]
...
class SimpleNetz[ParamType <: FloatNN: Default](input_size: Int, hidden_size: Int, output_size: Int)
    extends TensorModule[ParamType]
    with HasParams[ParamType] {
...
//  @main
    def main(): Unit = {
      ...
      // 01
      // 示例层
      val conv_layer = nn.Conv2d[Float32](
        in_channels = 3,
        out_channels = 16,
        kernel_size = 3,
        stride = 1,
...
//    val linear_layer = nn.Linear(in_features =???, out_features = 10) // 问题：in_features 是什么？
      ...
      // 02
      val device =
        if torch.cuda.is_available() then torch.Device("cuda")
...
//    println(s"Model device: ${model.parameters().device}")
      ...
      else println("GPU 不可用，无法创建 gpu_tensor。")
